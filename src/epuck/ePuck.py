...
class ePuck():
    ...
    def __init__(self, port, debug=False):
        ...
        :type	device: serial port device
    ...
    def _debug(self, *txt):
        ...
        if self.debug:
            print >> sys.stderr, '\033[31m[ePuck'+str(self.port)+']:\033[0m ', ' '.join([str(e) for e in txt])
    ...
    def _recv(self, n=4096):
        ...
        #self._debug("Request to read " + str(n) + " bytes")
    ...
    def _read_sensors(self):
        ...
        def send_binary_mode(parameters):
            ...
            # Parameters: ('Char to be sent', 'Size of reply waited', 'Format of the reply')
            ...
            reply = ()
            try:
                reply = self._recv(parameters[1])
                while len(reply) < parameters[1]:
                    reply += self._recv(parameters[1]-len(reply))
    ...
    def connect(self):
        ...
        if self.conexion_status:
            ...
            return False
        try:
            self.serial = serial.Serial(self.port, 230400, timeout=0)  # open serial port
